...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
        ...
        <p>
            <tr>
                <td align="left" valign="top" class="paragraph-mode">
                    <p>[<xsl:value-of select="count(preceding-sibling::paragraph) + 1"/>]</p>
                </td>
                <td align="left" valign="top" class="paragraph-mode">
                    <p>
                        <xsl:apply-templates select="s/kar" mode="paragraph"/>
                    </p>
                </td>
                <td/>
                <td align="left" valign="top" class="paragraph-mode">
                    <p>
                        <xsl:apply-templates select="s/eng"/>
                    </p>
                </td>
            </tr>
        </p>
    </xsl:template>
    <xsl:template match="kar" mode="paragraph">
        <b>
        ...
        <p>
            <table border="0" cellpadding="0" cellspacing="0">
                ...
                <tr>
                    ...
                    <td align="left">
                        <span class="karuk">
                            <xsl:apply-templates select="kar" mode="paragraph"/>
                        </span>
                    </td>
                </tr>
                <tr>
        ...
        <p>
            <table border="0" cellpadding="0" cellspacing="0">
                ...
                <tr>
                    <td align="left" rowspan="2" valign="top"> [<xsl:value-of
                            select="count(preceding::s[ancestor::data=$data-ancestor]) + 1"/>] </td>
                    <xsl:apply-templates select="kar" mode="word-karuk"/>
                </tr>
                <tr>
                    <xsl:apply-templates select="kar" mode="word-english"/>
                </tr>
            </table>
            <table border="0" cellpadding="0" cellspacing="0">
                <col width="50"/>
    ...
    <xsl:template match="kar" mode="word-karuk">
    ...
    <xsl:template match="kar" mode="word-english">
    ...
    <xsl:template match="w" mode="word-english">
        <td align="left">
            <xsl:choose>
                ...
                <xsl:when test="@language='english' or @language='nonsense' or @language='yurok'">
                    <i>
                        <xsl:value-of select="."/>
                    </i>
                </xsl:when>
        ...
        <p>
            <table border="0" cellpadding="0" cellspacing="0">
                ...
                <tr>
                    <td align="left" rowspan="2" valign="top"> [<xsl:value-of
                            select="count(preceding::s[ancestor::data=$data-ancestor]) + 1"/>] </td>
                    <xsl:apply-templates select="kar" mode="gloss-karuk"/>
                </tr>
                <tr>
                    <xsl:apply-templates select="kar" mode="gloss-english"/>
                </tr>
            </table>
            <table border="0" cellpadding="0" cellspacing="0">
                <col width="50"/>
    ...
    <xsl:template match="kar" mode="gloss-karuk">
        <xsl:apply-templates select="w" mode="gloss-karuk"/>
    </xsl:template>
    <xsl:template match="kar" mode="gloss-english">
    ...
    <xsl:template match="m" mode="gloss-karuk">
        <xsl:choose>
            <xsl:when test="not(@id) or (@id='0')">
                <xsl:apply-templates/>
            </xsl:when>
            <xsl:otherwise>
                <a href="{concat('karuk-dictionary.php?lxGroup-id=',@id)}">
                    <xsl:apply-templates/>
                </a>
            </xsl:otherwise>
        </xsl:choose>
    ...
    <xsl:template match="karuk-plain"/>
</xsl:stylesheet>
