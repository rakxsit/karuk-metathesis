...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
            ...
            <body>
                ...
                <div class="menu">
                    <ul>
                        ...
                        <xsl:choose>
                            <xsl:when test="$list='yes' and $tag=''">
                                <li class="on">Sentences &#38; Texts</li>
                            </xsl:when>
                            <xsl:otherwise>
                                <li>
                                    <a href="karuk-texts.php?list=yes">Sentences &#38; Texts</a>
                                </li>
                            </xsl:otherwise>
                        </xsl:choose>
                        <li>
                            <a href="project.php">Our Project</a>
                        </li>
                        <li>
                            <a href="karuk-thesaurus.php">Audio Thesaurus</a>
                        </li>
                        <li>
                            <a href="abbreviations.php">Abbreviations &#38; Links</a>
                        </li>
                        <!--   <li>
                ...
                <div class="content">
                    <xsl:choose>
                        <xsl:when test="$list='yes'">
                            ...
                            <xsl:if
                                test="document('karuk-texts.xml')/texts/text/metadata/tag/attribute::type">
                                <p>Click on one of the following for sentences or texts related to a
                                    given theme:</p>
                                <ul>
                                    <xsl:if
                                        test="document('karuk-texts.xml')/texts/text/metadata/tag/@type='topic'">
                                        <li>
                                            <small>Topics: <xsl:apply-templates mode="tags"
                                                  select="document('karuk-tags.xml')/tags/topics/tag"
                                                  ><xsl:sort select="@tag"
                                                /></xsl:apply-templates></small>
                                        </li>
                                    </xsl:if>
                                    <xsl:if
                                        test="document('karuk-texts.xml')/texts/text/metadata/tag/@type='grammar'">
                                        <li>
                                            <small>Grammar: <xsl:apply-templates mode="tags"
                                                  select="document('karuk-tags.xml')/tags/grammar/tag"
                                                  ><xsl:sort select="@tag"
                                                /></xsl:apply-templates></small>
                                        </li>
                                    </xsl:if>
                            ...
                            <xsl:choose>
                                <xsl:when test="not($full-list='yes')">
                                    <p>Click <a
                                            href="http://linguistics.berkeley.edu/~karuk/karuk-texts.php?list=yes&amp;full-list=yes"
                                            >here</a> to see a complete list of all texts and
                                        transcribed sentence collections.</p>
                                </xsl:when>
                                <xsl:otherwise>
                                    <table>
                                        <tr>
                                            ...
                                            <td>
                                                <p>An audio icon indicates available audio
                                                  recordings. To sort, click on a column header. For
                                                  example, click on "Year" to sort by year.</p>
                                            </td>
                                        </tr>
                                    </table>
                                    <hr/>
                                    <table class="sortable">
                                        <thead>
                                            <tr>
                                                ...
                                                <td>
                                                  <small>
                                                  <a href="#">Title</a>
                                                  <xsl:if test="not($tag='')"> (related to
                                                  <b><xsl:value-of select="$tag"/></b>)</xsl:if>
                                                  </small>
                                                </td>
                                                <td>
                                                  <small>
                                        ...
                                        <xsl:choose>
                                            <xsl:when test="not($tag='')">
                                                <xsl:apply-templates
                                                  select="text[metadata/tag/@tag=$tag]" mode="list"
                                                />
                                            </xsl:when>
                                            <xsl:otherwise>
    ...
    <xsl:template match="txt" mode="paragraph">
        <b>
            <xsl:choose>
                <xsl:when
                    test="@language='english' or @lang='eng' or @language='yurok' or @language='nonsense'">
                    <xsl:value-of select="."/>
                </xsl:when>
                <xsl:otherwise>
    ...
    <xsl:template match="s" mode="word">
        ...
        <p>
            <xsl:choose>
                <xsl:when
                    test="txt/@language='english' or txt/@lang='eng' or txt/@language='yurok' or txt/@language='nonsense'">
                    <table border="0" cellpadding="0" cellspacing="0">
                        <col width="50"/>
                        <col/>
                        <tr>
                            <td align="left" rowspan="2" valign="top"> [<xsl:value-of
                                    select="count(preceding::s[ancestor::data=$data-ancestor]) + 1"
                                />] </td>
                            <td align="left">
    ...
    <xsl:template match="tag" mode="tags">
        <xsl:variable name="tag-name" select="@tag"/>
        <xsl:if test="document('karuk-texts.xml')/texts/text/metadata/tag/@tag=$tag-name">
            <xsl:choose>
                <xsl:when test="$tag-name=$tag"><b><xsl:value-of select="$tag-name"/></b> | </xsl:when>
                <xsl:otherwise><a
                        href="{concat('karuk-texts.php?list=yes&amp;full-list=yes&amp;tag=',$tag-name)}"
                            ><xsl:value-of select="$tag-name"/></a> | </xsl:otherwise>
            </xsl:choose>
        </xsl:if>
    </xsl:template>
    <xsl:template match="text-plain"/>
    <xsl:template match="dependent-lemmas"/>
</xsl:stylesheet>
